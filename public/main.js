...
fetch(url)
  ...
  .then((data) => getOpen(data));

function getOpen(data) {
  let locale = navigator.language;
  let oh = new opening_hours(data["elements"][0]["tags"]["opening_hours"], {}, { locale: locale });
  let icon = "";
  let color = "";
  ...
  renderLinkIcon(elements, icon, color);
  return is_open;
}
...
function renderLinkIcon(node, icon, color) {
