...
class OpenstreetmapElement {
    ...
    getOsmElementbyTypeId(type: string, id: number) {
        const element = this.overpassturboElements.find(
    ...
    setAttributes(element: HTMLElement, object: OsmElement, is_open: string) {
        element.setAttribute("data-opening_hours", object["tags"]["opening_hours"]);
        element.setAttribute("data-is_open", is_open);
    }

    setText(element: HTMLElement, name: string) {
        const span = document.createElement("span");
        span.textContent = name;
        element.appendChild(span);
    }

    setOpenIcon(element: HTMLElement, open: boolean) {
    ...
    setIcon(element: HTMLElement, code: string) {
        const iconSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        ...
        iconTag.setAttribute("class", code);
